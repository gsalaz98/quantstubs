...
def build_signature(p_name, params, method_return=None):
    if any([True for i in params if '`' in i]):
        print('DEBUG:::: {} {} {}'.format(p_name, params, method_return))
...
def restore_clr(p_name, p_class):
    ...
    for m in methods:
        ...
        parameter_types.append([resolve_generic_type_params(t.ParameterType) for t in m.GetParameters()])
        return_type = m.ReturnType if m.Name == 'MethodInfo' else None
        method_returns.append([resolve_generic_type_params(return_type)])
...
def resolve_generic_type_params(clr_type):
    if clr_type is None:
        return 'None'
    ...
    if type_arg_count == 0 and clr_type.IsGenericType:
        print(clr_type.Name)

    return clr_type.Name.split('`')[0] + "[" + ', '.join([resolve_generic_type_params(t) for t in generic_args]) + "]"